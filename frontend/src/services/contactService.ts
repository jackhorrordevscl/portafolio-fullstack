...
/**
 * Simula el envío del formulario (útil para desarrollo)
 */
export const sendContactFormMock = async (
  data: ContactFormData,
): Promise<ContactResponse> => {
  // Validar datos
  const validationErrors = validateContactForm(data);
  ...
  console.log("📧 Mock - Formulario enviado:", data);

  return {
    success: true,
    message: "¡Mensaje enviado con éxito! (Modo desarrollo)",
  };
};

// ────────────────────────────────────────────────────────────
// Export principal con switch entre mock y real
// ────────────────────────────────────────────────────────────

/**
 * Función principal para enviar contacto
 * Usa mock en desarrollo si no hay backend disponible
 */
export const submitContactForm = async (
  ...
  if (useMock || import.meta.env.VITE_USE_MOCK_API === "true") {
    return sendContactFormMock(sanitizedData);
  }

  return sendContactForm(sanitizedData);
};
